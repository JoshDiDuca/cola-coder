...
class TestContentFilter:
    ...
    def test_autogen_threshold_default_rejects_single_marker(self):
        # Default (max_autogen_markers=0): a single marker rejects.
        code = "// @generated\n" + "const x = 1;\n" * 20
        keep, reason = self.f.check(FakeRecord(content=code))
        assert keep is False and "autogenerated" in reason

    def test_autogen_threshold_tolerates_up_to_max(self):
        # DATA-016: max_autogen_markers must actually be honored (it was a no-op
        # — any single marker rejected regardless of the configured threshold).
        from cola_coder.data.filters.content import ContentFilter

        f = ContentFilter(max_autogen_markers=1)
        one = "// @generated\n" + "const x = 1;\n" * 20
        assert f.check(FakeRecord(content=one))[0] is True  # 1 marker tolerated
        two = "// @generated\n// do not edit\n" + "const x = 1;\n" * 20
        keep, reason = f.check(FakeRecord(content=two))
        assert keep is False and "autogenerated" in reason  # 2 markers > 1
